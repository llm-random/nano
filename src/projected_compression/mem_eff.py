...
class MemoryEfficientProjectedCompression(nn.Module):
    ...
    def pass_gradient_to_projections(self, optimizers: List, schedulers, gradient_clipping):
        ...
        def get_compressed_params_grad_norm(block):
            ...
            grads = [p.weight.grad for p in compressed_params if p.weight.grad is not None]
        ...
        self.backward_compressed_weights(self.projections.head, self.source_model.head.linear.weight, self.target_model.head.linear.weight)
        ...
        if optimizers is None:
            ...
            for block_target, block_source, block_proj, optimizer, scheduler in zip(
                ...
                compressed_params_grad_norm = get_compressed_params_grad_norm(block_target)
                ...
                if self.adjust_grad_norm:
                    grad_norm_to_use = (start_grad_norm**2 - compressed_params_grad_norm**2 + projection_block_grad_norm**2)**0.5
               
                torch.nn.utils.clip_grads_with_norm_(block_proj.parameters(), gradient_clipping, grad_norm_to_use)
            ...
            grads = [v.grad for v in self.parameters() if v.grad is not None]
            final_grad_norm = torch.nn.utils.get_total_norm(grads + projection_blocks_grad_norms)
...
class CompressibleBlock(nn.Module):
    ...
    def init_projection_weights(self, dmodel_topk_indices, dff_topk_indices):
        self.compressible_q.init_projection_weights(dmodel_topk_indices, dmodel_topk_indices)
        ...
        self.compressible_ff_pre.init_projection_weights(dmodel_topk_indices, dff_topk_indices)
        self.compressible_ff_gate.init_projection_weights(dmodel_topk_indices, dff_topk_indices)
        self.compressible_ff_post.init_projection_weights(dff_topk_indices, dmodel_topk_indices)
...
class Projections(nn.Module):
    ...
    def init_projection_weights(self, path_to_importances):
        dmodel_topk_indices, dff_topk_indices = get_topk_indices(path_to_importances, self.target_dmodel, self.target_dff)
        ...
        with torch.no_grad():
            ...
            self.auxiliary_embedding_weights.weight.data.copy_(torch.zeros_like(self.auxiliary_embedding_weights.weight.data))

            for block, block_dff_topk_indices in zip(self.blocks, dff_topk_indices):
                block.init_projection_weights(dmodel_topk_indices, block_dff_topk_indices)
