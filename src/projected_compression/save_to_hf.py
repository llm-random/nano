...
def load_pc_state_dict_to_llama(state_dict, original_llama):
    ...
    conf.hidden_size = state_dict['encoder.blocks.0.attention_layer.norm.weight'].shape[0]
    conf.intermediate_size = state_dict['encoder.blocks.0.ff_layer.layer.gate.weight'].shape[0]
...
def save_pc_to_hf(cfg, metric_logger):
    ...
    model = create_model(cfg.model, cfg.projected_compression, source_model_for_distillation=False)

    dcp.load(model.state_dict(), checkpoint_id=cfg.trainer.checkpoint.load.path)

    model.prepare_compressed_weights()
    model.target_model.embedding = torch.nn.Parameter(model.source_model.embedding.weight @ model.projections.embedding.T + model.projections.auxiliary_embedding_weights.weight)
    ...
    if os.environ.get("RANK", "0") == "0":
        llama_model = load_pc_state_dict_to_llama(target_model_state_dict, original_llama=cfg.projected_compression.original_llama_path)
        llama_model.save_pretrained(cfg.trainer.checkpoint.save.path)
        tokenizer = AutoTokenizer.from_pretrained(cfg.projected_compression.original_llama_path)
