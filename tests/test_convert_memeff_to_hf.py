...
def _make_state_dict(dmodel, dff, n_layers, q_heads, kv_heads, head_dim, vocab_size):
    sd = {
        ...
        "head.linear.weight": torch.full((vocab_size, dmodel), 2.0),  # deliberately different
    }
    for layer in range(n_layers):
        p = f"encoder.blocks.{layer}."
        sd[f"{p}attention_layer.norm.weight"] = torch.ones(dmodel)
        sd[f"{p}attention_layer.layer.q_proj.weight"] = torch.ones(q_heads * head_dim, dmodel)
        sd[f"{p}attention_layer.layer.k_proj.weight"] = torch.ones(kv_heads * head_dim, dmodel)
        sd[f"{p}attention_layer.layer.v_proj.weight"] = torch.ones(kv_heads * head_dim, dmodel)
        sd[f"{p}attention_layer.layer.o_proj.weight"] = torch.ones(dmodel, q_heads * head_dim)
...
def test_embedding_not_corrupted_for_1b():
    ...
    sd = _make_state_dict(
        dmodel=16, dff=32, n_layers=16,
        q_heads=32, kv_heads=8, head_dim=64,
        vocab_size=128256,
    )
    ...
    assert torch.all(emb == 1.0), "embed_tokens.weight was corrupted (expected all-ones)"
    assert torch.all(lm_head == 2.0), "lm_head.weight was not loaded correctly (expected all-twos)"
    assert not torch.equal(emb, lm_head), "embed_tokens and lm_head should be independent"


def test_embedding_not_corrupted_for_8b():
    """Same check for 8B — should already work, confirms no regression."""
    sd = _make_state_dict(
        dmodel=16, dff=32, n_layers=32,
