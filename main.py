...
from src.core.checkpointing import load_training_state
...
from src.core.checkpointing import load_model_state_dict, load_optimizer, load_scheduler, save_checkpoint, get_full_checkpoint_path
...
def run(cfg:OmegaConf, metric_logger=None):
    ...
    state_dict = {}
    if cfg.trainer.checkpoint.load.path is None and os.environ["RANK"] == "0":
        model = instantiate(cfg.model, _convert_="all")
        logger.debug(f"Model {model.__class__.__name__} created with {sum(p.numel() for p in model.parameters() if p.requires_grad)} random trainable parameters")
    else:
        with torch.device('meta'):
            model = instantiate(cfg.model, _convert_="all")
            logger.debug(f"Model {model.__class__.__name__} initialized with {sum(p.numel() for p in model.parameters() if p.requires_grad)} trainable parameters")

        for fn in instantiate(cfg.get("apply_functions_before_load", [])):
            fn(model)
        ...
        for fn in instantiate(cfg.get("apply_functions_after_load", [])):
            fn(model)

        if os.environ["RANK"] == "0": 
            state_dict = model.state_dict()
            logger.info(f"Model {model.__class__.__name__} ready with {sum(p.numel() for p in model.parameters() if p.requires_grad)} trainable parameters")
    ...
    if cfg.trainer.checkpoint.save.path is not None:
        for fn in instantiate(cfg.get("apply_functions_after_train", [])):
