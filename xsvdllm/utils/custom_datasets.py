...
class AbstractDataset(IterableDataset):
    ...
    def __init__(
        ...
        limit: Optional[int] = None, # [SAFE CHANGE] Added optional limit
    ):
    ...
    def __iter__(self):
        ...
        # 1. Get base iterator
        iterator = self.sample_packer()
        
        # 2. Handle Sharding
        if self.world_size_independent:
            iterator = itertools.islice(iterator, self.rank, None, self.world_size)
            
        # 3. [SAFE CHANGE] Apply Limit if requested
        if self.limit is not None:
            iterator = itertools.islice(iterator, self.limit)
...
class FineWebEduDataset(AbstractDataset):
    ...
    def __init__(
        ...
        shuffle: bool = True,
        world_size_independent: bool = False,
        limit: Optional[int] = None # [SAFE CHANGE] Pass through
    ):
...
class C4Dataset(AbstractDataset):
    ...
    def _load_dataset(self, path, split, seed, tokenize_fn, shuffle: bool):
        if path is None:
            logger.debug(f"Loading 'allenai/c4' dataset from HuggingFace with split={split}")
            hf_dataset = load_dataset("allenai/c4", "en", split=split, streaming=True, trust_remote_code=True)
        else:
            logger.debug(f"Loading dataset from path '{path}'")
            hf_dataset = load_from_disk(path)
            hf_dataset = hf_dataset.to_iterable_dataset(num_shards=self.NUM_SHARDS)
        ...
        if not self.world_size_independent:
            hf_dataset = split_dataset_by_node(hf_dataset, rank=self.rank, world_size=self.world_size)
...
# [NO CHANGES NEEDED BELOW THIS LINE - get_dataloader remains exactly the same]
def get_dataloader(
    dataset_type: str,
    dataset_path: str,
    dataset_split: str,
